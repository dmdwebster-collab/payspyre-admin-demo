...
import { SERVICING_TABS } from "@/lib/servicing-tabs";
...
/**
 * Servicing Loan Header tab strip. Client component for active-tab
 * highlighting; everything else in the layout stays a server component.
 * Mirrors components/originations/loan-header-tabs.tsx.
 */
export function ServicingLoanHeaderTabs({ loanId }: Props) {
  const pathname = usePathname();
  return (
    <nav
      aria-label="Servicing Loan Header tabs"
      className="border-b border-line bg-navy-900 px-6 sticky top-[88px] z-10"
    >
      <ul className="flex gap-1 overflow-x-auto">
        {SERVICING_TABS.map((tab) => {
          const href = `/servicing/${loanId}/${tab.slug}`;
